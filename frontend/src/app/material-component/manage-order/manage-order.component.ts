...
export class ManageOrderComponent implements OnInit {
  ...
  add(){
    ...
    if(productName == undefined) {
      ...
      this.dataSource.push({id:formData.product.id,name:formData.product.product_name,category:formData.category.category_name,
        quantity: formData.quantity, price: formData.price, total: formData.total
      })
      this.dataSource = [...this.dataSource]
      this.snackbarService.openSnackBar(GlobalConstants.productAdded, "Close")
    }
    else{
  ...
  submitAction(){
    ...
    this.billService.insertBill(data).subscribe((res:any)=>{
      this.manageOrderForm.reset()
      this.totalAmount = 0
      this.dataSource = []
      this.snackbarService.openSnackBar("Order Placed", "Close")
      this.ngxService.stop()
    },(err:any)=>{
      this.ngxService.stop()
